...
connection = MongoClient("mongodb://tagging-database:27017/?appname=tagging_service&ssl=false")
...
class Clusters(Resource):
    ...
    def get(self, dataset_id, question_id, user_id):
        return get_cluster(connection=connection, dataset_id=dataset_id, question_id=question_id, user_id=user_id)[0]

    @api.expect(CLUSTERED_ANSWERS)
    def post(self, dataset_id, question_id, user_id):
        data = request.get_json()
        save_cluster(connection=connection, dataset_id=dataset_id, question_id=question_id, user_id=user_id,
                     cluster=data)
